...
# 文件路径
FILE_PATH = os.path.join(os.path.expanduser("~"), "Downloads", "Weekly Report Data Extracted_processed_21072025_FINAL_Test.xlsx")
            ...
            for i, song in enumerate(suggestions):
                with cols[i % 2]:  # 交替分配到两列
                    if st.button(song, key=f"suggest_{song}"):
                        ...
                        st.rerun()  # 使用st.rerun()替代已弃用的experimental_rerun
